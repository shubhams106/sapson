...
const QueriesAll = async ({ searchParams }: SearchParams) => {
  ...
  if (
    !session ||
    (session?.user?.email !== "singlas106@gmail.com" &&
      session?.user?.email !== "sapsonpharma@gmail.com")
  ) {
    redirect("/");
  }
  ...
  return (
    <>
      <section className="flex w-full flex-col-reverse justify-between gap-4 sm:flex-row sm:items-center max-w-5xl mx-auto  ">
